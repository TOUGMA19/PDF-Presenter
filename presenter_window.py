...
QPushButton#welcomeOpen {{
    background: qlineargradient(x1:0, y1:0, x2:1, y2:1,
        stop:0 {ACCENT}, stop:0.5 #5e9aff, stop:1 {ACCENT2});
    color: white;
...
QPushButton#welcomeOpen:hover {{
    background: qlineargradient(x1:0, y1:0, x2:1, y2:1,
        stop:0 #6aa8ff, stop:0.5 #79b2ff, stop:1 #9e7dff);
    border: 1px solid rgba(255,255,255,0.30);
}}
QPushButton#welcomeOpen:pressed {{
    background: qlineargradient(x1:0, y1:0, x2:1, y2:1,
        stop:0 #3a70e0, stop:1 #6040cc);
}}
QPushButton#welcomeSecondary {{
    background: rgba(255,255,255,0.06);
    color: rgba(200,210,240,0.90);
...
QPushButton#welcomeSecondary:pressed {{
...
QPushButton#toolbarOpen:hover {{
    background: qlineargradient(x1:0,y1:0,x2:1,y2:1,stop:0 #6aa8ff,stop:1 #9e7dff);
}}
QPushButton#toolbarNav {{
...
QPushButton#toolbarNav:hover {{
    background: rgba(124,92,252,0.26);
    border-color: rgba(124,92,252,0.6);
}}
QPushButton#toolbarTimer {{
...
QPushButton#toolbarTimer:hover {{
    background: rgba(52,211,153,0.22);
    border-color: rgba(52,211,153,0.55);
}}
QPushButton#toolbarFull {{
...
QListWidget#recents::item:selected {{
    background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
        stop:0 rgba(78,140,255,0.28), stop:1 rgba(124,92,252,0.28));
    color: white;
...
class _RecentItemDelegate(QStyledItemDelegate):
    """Custom delegate: PDF icon pill + filename (bold) + folder path (muted)."""

    _PDF_BG   = QColor(78, 140, 255, 45)
    _PDF_BORDER = QColor(78, 140, 255, 120)
    _PDF_FG   = QColor(100, 160, 255)
    _NAME_COL = QColor(238, 240, 248)
    _PATH_COL = QColor(107, 114, 128)
    ...
    def paint(self, painter, option, index):
        ...
        if option.state & QStyle.State_Selected:
            ...
            bg.setColorAt(0, QColor(78, 140, 255, 55))
            bg.setColorAt(1, QColor(124, 92, 252, 55))
            path = QPainterPath()
            path.addRoundedRect(QRectF(rect), 8, 8)
            painter.fillPath(path, QBrush(bg))
            painter.setPen(QPen(QColor(78, 140, 255, 100), 1))
            painter.drawPath(path)
        elif option.state & QStyle.State_MouseOver:
            path = QPainterPath()
            path.addRoundedRect(QRectF(rect), 8, 8)
            painter.fillPath(path, QBrush(QColor(37, 40, 64, 200)))
            painter.setPen(QPen(QColor(42, 45, 62, 255), 1))
            painter.drawPath(path)

        # ── PDF pill icon with gradient ──
        pill = QRectF(rect.left() + 10, rect.top() + (rect.height() - 30) / 2, 46, 30)
        pill_path = QPainterPath()
        pill_path.addRoundedRect(pill, 7, 7)
        pill_grad = QLinearGradient(pill.left(), pill.top(), pill.right(), pill.bottom())
        pill_grad.setColorAt(0.0, QColor(78, 140, 255, 200))
        pill_grad.setColorAt(1.0, QColor(124, 92, 252, 200))
        painter.fillPath(pill_path, QBrush(pill_grad))
        painter.setPen(QPen(QColor(255, 255, 255, 50), 1))
        painter.drawPath(pill_path)
        painter.setPen(QColor(255, 255, 255, 240))
        font_pill = painter.font()
        font_pill.setPointSize(8)
        font_pill.setWeight(QFont.Bold)
        painter.setFont(font_pill)
        painter.drawText(pill, Qt.AlignCenter, "PDF")

        # ── text area ──
        tx = rect.left() + 68
        tw = rect.right() - tx - 8

        name = index.data(Qt.UserRole + 1) or ""
        folder = index.data(Qt.UserRole + 3) or ""

        # filename (bold, normal color)
        fn = QFont()
        fn.setPointSize(13)
        fn.setWeight(QFont.DemiBold)
        painter.setFont(fn)
        painter.setPen(self._NAME_COL)
        fm_name = QFontMetrics(fn)
        name_elided = fm_name.elidedText(name, Qt.ElideRight, int(tw))
        name_y = rect.top() + (rect.height() // 2) - 3
        painter.drawText(int(tx), name_y, name_elided)

        # folder (small, muted)
        fp = QFont()
        fp.setPointSize(10)
        painter.setFont(fp)
        painter.setPen(self._PATH_COL)
        fm_path = QFontMetrics(fp)
        path_elided = fm_path.elidedText(folder, Qt.ElideLeft, int(tw))
        path_y = rect.top() + (rect.height() // 2) + fm_path.height() - 1
        painter.drawText(int(tx), path_y, path_elided)
...
class WelcomeView(QWidget):
    ...
    def _build_ui(self):
        ...
        drag_hint.setStyleSheet(
            f"color: {MUTED}; font-size: 12px; background: transparent; letter-spacing: 0.3px;"
        )

        # ── Recent files ──
        recents_label = QLabel("📂  FICHIERS RÉCENTS")
        recents_label.setObjectName("welcomeSection")
        recents_label.setStyleSheet(
            f"color: {MUTED}; font-size: 11px; letter-spacing: 2.5px; font-weight: 800;"
            "background: transparent; padding-bottom: 4px;"
        )

        self.recents_list = QListWidget()
        self.recents_list.setObjectName("recents")
        self.recents_list.setMaximumHeight(420)
        self.recents_list.setUniformItemSizes(True)
        self.recents_list.setFocusPolicy(Qt.NoFocus)
        self.recents_list.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
    ...
    def refresh_recents(self, recents: list[str]):
        ...
        for p in recents:
            path = Path(p)
            name   = path.stem          # filename sans extension
            ext    = path.suffix        # ".pdf"
            parent = str(path.parent)
            item = QListWidgetItem()
            item.setData(Qt.UserRole, p)
            item.setData(Qt.UserRole + 1, name)
            item.setData(Qt.UserRole + 2, ext)
            item.setData(Qt.UserRole + 3, parent)
            item.setSizeHint(QSize(0, 66))
            self.recents_list.addItem(item)

        # install custom delegate once
        if not hasattr(self, "_recent_delegate"):
            self._recent_delegate = _RecentItemDelegate(self.recents_list)
            self.recents_list.setItemDelegate(self._recent_delegate)
...
class PresenterWindow(QWidget):
    def __init__(self, controller):
        ...
        btn_open.setObjectName("toolbarOpen")
        btn_open.clicked.connect(self.controller.open_file_dialog)
        btn_home  = QPushButton("⌂  Accueil")
        btn_home.setObjectName("homeBtn")
        btn_home.clicked.connect(self.controller.go_home)
        btn_goto  = QPushButton("↗  Aller à…");      btn_goto.setObjectName("toolbarNav"); btn_goto.clicked.connect(self.ask_goto)
        btn_pause = QPushButton("⏸  Pause");   btn_pause.setObjectName("toolbarTimer"); btn_pause.clicked.connect(self.controller.toggle_timer)
        self.btn_pause = btn_pause
        btn_reset = QPushButton("↺  Reset");   btn_reset.setObjectName("toolbarTimer"); btn_reset.clicked.connect(self.controller.reset_timer)
        btn_full  = QPushButton("⛶  Plein écran"); btn_full.setObjectName("toolbarFull"); btn_full.clicked.connect(self.controller.toggle_public_fullscreen)
        ...
        # Zoom combo for normal PDFs (articles, documents)
        self.zoom_combo = QComboBox()
        self.zoom_combo.setObjectName("zoomCombo")
        self.zoom_combo.setToolTip("Zoom pour PDFs standard")
        self._zoom_map = {0: "fit", 1: "fill", 2: "1.25", 3: "1.5", 4: "2.0"}
        self.zoom_combo.addItems(["Fit page", "Remplir", "125%", "150%", "200%"])
        self.zoom_combo.currentIndexChanged.connect(self._on_zoom_changed)
        self.zoom_combo.setVisible(False)

        self.zoom_label = QLabel("Zoom :")
        self.zoom_label.setObjectName("title")
        ...
        top.addSpacing(8)
        top.addWidget(self.zoom_label); top.addWidget(self.zoom_combo)
        top.addSpacing(10)
    ...
    def sync_mode_selector(self):
        ...
        # Show/hide zoom based on layout
        from pdf_document import LAYOUT_NORMAL
        show_zoom = (doc.layout == LAYOUT_NORMAL) or (doc.mode == "normal")
        self.zoom_combo.setVisible(show_zoom)
        self.zoom_label.setVisible(show_zoom)

        if show_zoom:
            self.zoom_combo.blockSignals(True)
            rev_map = {v: k for k, v in self._zoom_map.items()}
            zidx = rev_map.get(doc.zoom_mode, 0)
            self.zoom_combo.setCurrentIndex(zidx)
    ...
    def _on_zoom_changed(self, index: int):
        zoom = self._zoom_map.get(index, "fit")
        if self.controller.doc is not None:
            self.controller.set_zoom_mode(zoom)
